...
新动作素材入口是 `reference-library/actions/index.html`，原有人物参考入口是 `assets/character-references/index.html`，均为生成后的本地浏览页。新素材库迁移保留分组名称和组内相对路径；历史报告中的绝对路径只代表当时位置。临时生成输入、提示词和试产结果仍在 `.pet-work/`，需要保留的最终验收证据放在 `artifacts/pet-qa/`。
